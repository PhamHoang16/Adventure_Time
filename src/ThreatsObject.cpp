...
void ThreatsObject::set_clips()
{
    ...
    frame_clip_[10].x = 11*width_frame_;
    frame_clip_[10].y = 0;
    frame_clip_[10].w = width_frame_;
    frame_clip_[10].h = height_frame_;

    frame_clip_[11].x = 10*width_frame_;
    frame_clip_[11].y = 0;
...
void ThreatsObject::CheckMap(Map& map_data)
{
    ...
    if(x1 >=0 && x2 <= MAX_MAP_X && y1 >= 0 && y2 < MAX_MAP_Y){
        if(x_val_ > 0)   // hero moving to right
        {
            ...
            if((val1 != BLANK_TILE && val1 != GOLD)||(val2 != BLANK_TILE && val2 != GOLD)){
                x_pos_ = x2 * TILE_SIZE;
    ...
    for(int i = 0; i < bullet_list_.size(); i++)
    {
        ...
        if(p_bullet != NULL)
        {
            if(p_bullet->get_is_move())
            {
                ...
                if(bullet_distance < 1200 && bullet_distance > 0)
                {
