...
void MainObject::set_clip(){
    ...
    frame_clip_[8].x = 7*width_frame_;
    frame_clip_[8].y = 0;
    frame_clip_[8].w = width_frame_;
    frame_clip_[8].h = height_frame_;

    frame_clip_[9].x = 7*width_frame_;
    frame_clip_[9].y = 0;
...
void MainObject::HandleInputAction(SDL_Event events, SDL_Renderer* screen){
     ...
     if(events.type == SDL_MOUSEBUTTONDOWN)
    {
        ...
        if(events.button.button == SDL_BUTTON_LEFT)
        {
            ...
            p_bullet->set_bullet_type(BulletObject::BONE_BULLET);
...
{
    map_data.start_x_ = x_pos_ - (SCREEN_WIDTH/4);    //start x : điểm mép trái của màn hình
    if(map_data.start_x_ < 0)                         //x_pos : vị trí hiện tại của nhân vật
    {
        map_data.start_x_ = 0;
    }
...
void MainObject::CheckMap(Map& map_data)
{
    ...
    if(x1 >=0 && x2 <= MAX_MAP_X && y1 >= 0 && y2 < MAX_MAP_Y){
        if(x_val_ > 0)   // hero moving to right
        {
            int val1 = map_data.tile[y1][x2];
            int val2 = map_data.tile[y2][x2];

            if(val1 == GOLD || val2 == GOLD)
            {
        ...
        else if(x_val_ < 0)   // hero moving to right
        {
    ...
    if(x1 >= 0 && x2 < MAX_MAP_X && y1 >= 0 && y2 < MAX_MAP_Y)
    {
        ...
        else if(y_val_ < 0)
        {
            ...
            if(val1 == GOLD || val2 == GOLD || val1 == HEART || val2 == HEART)
            {
                map_data.tile[y1][x1] = 0;
                map_data.tile[y1][x2] = 0;
            }
